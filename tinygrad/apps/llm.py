...
class TransformerBlock:
  def __init__(self, dim:int, hidden_dim:int, n_heads:int, n_kv_heads:int, norm_eps:float, head_dim:int, rope_theta:float,
               ...
               qk_nope_head_dim:int=0, qk_rope_head_dim:int=0, v_head_dim:int=0, n_shared_experts:int=0, moe_hidden_dim:int=0,
    ...
    if not kv_lora_rank:
      ...
      self.attn_k_b = SimpleNamespace(weight=Tensor.empty(n_heads, kv_lora_rank, qk_nope_head_dim))
      self.attn_v_b = SimpleNamespace(weight=Tensor.empty(n_heads, v_head_dim, kv_lora_rank))
    ...
    if num_experts > 0:
      ...
      self.ffn_gate_exps = ExpertWeights(num_experts, dim, moe_hidden_dim or hidden_dim)
      self.ffn_up_exps = ExpertWeights(num_experts, dim, moe_hidden_dim or hidden_dim)
      self.ffn_down_exps = ExpertWeights(num_experts, moe_hidden_dim or hidden_dim, dim)
      self.exp_probs_b = SimpleNamespace(bias=Tensor.empty(num_experts))
      self.expert_weights_norm = expert_weights_norm
      self.expert_weights_scale = expert_weights_scale
      if n_shared_experts > 0:
        self.ffn_gate_shexp = nn.Linear(dim,  n_shared_experts * moe_hidden_dim, bias=False)
        self.ffn_up_shexp = nn.Linear(dim,  n_shared_experts * moe_hidden_dim, bias=False)
        self.ffn_down_shexp = nn.Linear(n_shared_experts * moe_hidden_dim, dim, bias=False)
    else:
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    if hasattr(self, 'kv_lora_rank'):
      ...
      q_nope_absorbed = (q_nope.unsqueeze(-2) @ self.attn_k_b.weight.transpose(-1, -2).unsqueeze(0).unsqueeze(2)).squeeze(-2)
      q = q_nope_absorbed.cat(q_pe, dim=-1)
      k_new = self.attn_kv_a_norm(compressed_kv).unsqueeze(1).cat(k_pe, dim=-1)
      if not hasattr(self, "cache_k"):
        cache_hidden_dim = self.kv_lora_rank + self.qk_rope_head_dim
        self.cache_k = Tensor.zeros((B, 1, self.max_context, cache_hidden_dim), dtype=k_new.dtype, device=k_new.device).contiguous().realize()
      self.cache_k[:, :, start_pos:start_pos+T, :].assign(k_new)
      k = self.cache_k[:, :, 0:start_pos+T, :]
      qk = q.matmul(k.transpose(-2, -1)) * self.q_head_dim ** -0.5
      ...
      attn_val = (qk.softmax(-1).matmul(k[:, :, :, :self.kv_lora_rank]) @ self.attn_v_b.weight.transpose(-1, -2)).transpose(1, 2).reshape(B, T, -1)
      return x + self.attn_output(attn_val)
  ...
  def _feed_forward(self, h: Tensor) -> Tensor:
    ...
    if hasattr(self, 'ffn_gate_exps'):
      ...
      if hasattr(self, 'kv_lora_rank'): # glm, need sigmoid, biased routing, and weight norm
...
class Transformer:
  def __init__(self, *, num_blocks, dim, hidden_dim, n_heads, n_kv_heads, norm_eps, vocab_size, head_dim:int, rope_theta:float,
    ...
    self.blk = [TransformerBlock(
      dim=dim, hidden_dim=hidden_dim, n_heads=n_heads, n_kv_heads=n_kv_heads, norm_eps=norm_eps, head_dim=head_dim, rope_theta=rope_theta,
      max_context=max_context, qk_norm=qk_norm, num_experts=0 if i < leading_dense_blocks else num_experts,
      num_experts_per_tok=num_experts_per_tok, kv_lora_rank=kv_lora_rank, q_lora_rank=q_lora_rank,
      qk_nope_head_dim=qk_nope_head_dim, qk_rope_head_dim=qk_rope_head_dim, v_head_dim=v_head_dim,
      n_shared_experts=n_shared_experts, moe_hidden_dim=moe_hidden_dim, expert_weights_norm=expert_weights_norm,
  ...
  def from_gguf(gguf:Tensor, max_context:int|None=None, realize=True) -> tuple[Transformer, dict]:
    ...
    hidden_dim = kv[f'{arch}.feed_forward_length'] if kv.get(f'{arch}.attention.kv_lora_rank') \
                    else kv.get(f'{arch}.expert_feed_forward_length', kv[f'{arch}.feed_forward_length'])
    model = Transformer(num_blocks=kv[f'{arch}.block_count'], dim=kv[f'{arch}.embedding_length'],
                        hidden_dim=hidden_dim, n_heads=n_heads, n_kv_heads=n_kv_heads, norm_eps=kv[f'{arch}.attention.layer_norm_rms_epsilon'],
                        ...
                        moe_hidden_dim=kv.get(f'{arch}.expert_feed_forward_length', hidden_dim),
...
models = {
  ...
  "glm4.7:30b": "https://huggingface.co/unsloth/GLM-4.7-Flash-GGUF/resolve/main/GLM-4.7-Flash-MXFP4_MOE.gguf",
}
